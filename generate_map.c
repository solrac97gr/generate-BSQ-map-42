...
void	generate_map(int rows, int cols, int obstacle_prob)
{
	...
	for (int i = 0; i < rows; i++)
	{
		for (int j = 0; j < cols; j++)
		{
			...
			{
				putchar(obstacle); // Print obstacle with probability
			}
			else
			{
				putchar(empty); // Print empty space
			}
		}
		if (i < rows - 1)
		{
...
int	main(int argc, char *argv[])
{
	...
	{
		fprintf(stderr,
			"Invalid input. Ensure rows and cols are positive integers and obstacle probability is between 0 and 100.\n");
		return (1);
	}
